...
| F5 | 地区別ルール（ハイブリッド） | 初回起動時に地区選択（MVPは8地区）。「現在地で確認」ボタンでオンデマンドGPS切替 |
...
飯田市のごみ収集地区は実際にはNo.36まで番号があり、**全市では推定30〜40地区** ある。MVPでは、以下の手順でカバーする：

1. **MVPフェーズ1（コンテスト・行政アピール用）**: 既にPDFから読み取った **8地区のみ対応**
   - No.1 大門町・桜町・大王路・小伝馬町1・錦町・東新町・諏訪町
   - No.10 扇町・水の手町・南常盤町・愛宕町
   - No.15 座光寺
   - No.17 水城・城
   - No.23 千代
   - No.25 竜丘・川路・三穂
...
```
[ようこそ画面] → [地区選択画面（8地区）] → [通知許可ダイアログ] → [ホーム画面]
```
...
| AreaSelect（初回のみ） | 8地区から自分の地区を選ぶ。「他のエリアは近日対応予定」を明記 |
...
- **現状**: PDFからAIが生成した草稿。8地区のPDFを精読しパターン化済みだが、No.36のパターンは要確認
- **公開前にやること**:
  - まさゆきさんが手元の8地区PDFと照らし合わせて手動校正（特にNo.36）
