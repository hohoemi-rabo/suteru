/**
 * これどう捨てる？ デザインシステム（デザイントークンの正本）
 *
 * トーン: 明るく・親しみやすく・ポップで生活感のある
 * 主役カラー: 緑（エコ・清潔感）+ 空色（情報・地区）
 *
 * 色・余白・角丸・文字サイズはハードコードせず、必ずこのファイル経由で参照する。
 * NativeWind の className を使う箇所は tailwind.config.js のトークン（このファイルと同期）を、
 * 直接 style/プロップに色を渡す箇所（タブバー等）はこの Palette を参照する。
 *
 * 設計仕様の原本: design/Colors.ts / design/UI-IMPROVEMENT.md
 */
